...
import type { Story, LoadingTarget, ChatMessage, Character } from './types';
import { GoogleGenAI, Chat } from '@google/genai';
import { generateConcept, generateChapterOutlines, generateChapterContent, narrateText, generateSynopsis, generateCoverImage, generateConceptArt, generateDescription, generateTags, generateRealisticCover } from './services/geminiService';
...
export default function App() {
    ...
    useEffect(() => {
        try {
            const savedStories = localStorage.getItem('ia-story-studio-stories');
            if (savedStories) setStories(JSON.parse(savedStories));
        } catch (e) {
            console.error("Failed to load stories:", e);
            setError("Não foi possível carregar suas histórias salvas.");
        }
    }, []);

    // Save stories to local storage with debounce and visual feedback
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            try {
                localStorage.setItem('ia-story-studio-stories', JSON.stringify(stories));
                setSaveStatus('saved');
    ...
    const handleGenerateContent = (chapterIndex: number) => {
        ...
        // Cria um contexto otimizado para evitar sobrecarga e repetição
        const previousChapters = activeStory.chapters.slice(0, chapterIndex);
        const recentChaptersContent = previousChapters.slice(-2) // Pega os últimos 2 capítulos
            .map((c, i) => `--- Conteúdo do Capítulo Anterior (${previousChapters.length - 1 + i}) ---\nTítulo: ${c.title}\n\n${c.content}\n--- Fim do Conteúdo ---`)
            .join('\n\n');

        const allChapterOutlines = activeStory.chapters
            .map((c, i) => `${i + 1}. ${c.title}: ${c.outline}`)
            .join('\n');
            
        const characterProfiles = activeStory.characters
    ...
    const handleDownloadAudio = (chapterIndex: number) => {
        ...
        // A API retorna 16-bit PCM a 24kHz, mono
    ...
    const handleExport = () => {
        if (stories.length === 0) return setError("Não há histórias para exportar.");
        const dataStr = JSON.stringify(stories, null, 2);
        const link = document.createElement('a');
        link.href = URL.createObjectURL(new Blob([dataStr], { type: 'application/json' }));
        link.download = 'estudio-de-historias.json';
        link.click();
    ...
    const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            try {
                const imported = JSON.parse(e.target?.result as string) as Story[];
                if (!Array.isArray(imported) || !imported.every(s => s.id && s.title !== undefined)) throw new Error("Invalid format");
                setStories(prev => {
    ...
    const handleSendChatMessage = async (message: string) => {
        ...
        try {
            if (message.toLowerCase().startsWith('/imagem ')) {
                ...
                if (!chatRef.current) openChat(); // Should be initialized already but as a fallback
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <header className="text-center mb-12">
                    <h1 className="text-4xl sm:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-600">
                        Estúdio de Histórias IA - Nova Versão
                    </h1>
                ...
                {activeStoryId === null ? (
                    ...
                    <div className="animate-fade-in">
                         <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                            <h2 className="text-3xl font-bold text-indigo-300">Minha Biblioteca</h2>
                            <div className="flex flex-col sm:flex-row items-center gap-4">
                                <SaveStatusIndicator status={saveStatus} />
                                <div className="flex gap-2">
                                    <input type="file" ref={fileInputRef} onChange={handleImport} accept=".json" className="hidden" />
                                    <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md text-white bg-gray-700 hover:bg-gray-600 transition-colors" title="Carregar histórias de um arquivo de backup (.json)">
                                    ...
                                    <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md text-white bg-gray-700 hover:bg-gray-600 transition-colors" title="Salvar todas as histórias em um arquivo de backup (.json)">
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            id="chapterCount"
                                            className="w-full bg-gray-900/70 rounded-lg p-2 focus:ring-2 focus:ring-indigo-500"
                                            value={activeStory.chapterCount}
                                            onChange={(e) => updateActiveStory(() => ({ chapterCount: Math.max(1, parseInt(e.target.value, 10) || 1) }))}
                                            min="1"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            id="wordCount"
                                            className="w-full bg-gray-900/70 rounded-lg p-2 focus:ring-2 focus:ring-indigo-500"
                                            value={activeStory.wordCount}
                                            onChange={(e) => updateActiveStory(() => ({ wordCount: Math.max(100, parseInt(e.target.value, 10) || 100) }))}
                                            min="100"
                                            step="100"
                                        />
                                    </div>
                                ...
                                <div className="bg-gray-800/50 p-6 rounded-2xl ring-1 ring-white/10 space-y-8">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            id="authorName"
                                            type="text"
                                            className="w-full bg-gray-900/70 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500"
                                            placeholder="Digite seu nome ou pseudônimo"
                                            value={activeStory.authorName}
                                            onChange={(e) => updateActiveStory(() => ({ authorName: e.target.value }))}
                                        />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        {activeStory.tags.length > 0 ? (
                                            <div className="flex flex-wrap gap-2">
                                                {activeStory.tags.map(tag => <span key={tag} className="text-xs bg-gray-700 text-gray-300 px-2.5 py-1 rounded-full">{tag}</span>)}
                                            </div>
                                        ) : <button onClick={handleGenerateTags} disabled={loading?.type === 'tags'} className="inline-flex items-center px-4 py-1.5 text-sm font-medium rounded-md text-white bg-purple-700 hover:bg-purple-800 disabled:bg-purple-900/50">{loading?.type === 'tags' ? <><Spinner/> Gerando...</> : 'Gerar 25 Tags'}</button>}
                                    </div>
                                    ...
                                    <div className="border-t border-white/10 pt-8">
                                        ...
                                        <div className="grid md:grid-cols-2 gap-8 items-start">
                                            ...
                                            <div className="text-center p-4 bg-gray-900/40 rounded-lg">
                                                ...
                                                {activeStory.userCoverImage && <img src={`data:${activeStory.userCoverImageMimeType};base64,${activeStory.userCoverImage}`} alt="Preview" className="w-24 mx-auto rounded-md mb-2"/>}
                                        ...
                                        <div className="mt-8">
                                            ...
                                            <div className="max-w-xs mx-auto aspect-[9/16] bg-gray-900/50 rounded-lg flex items-center justify-center ring-1 ring-white/10">
                                                {loading?.type === 'cover' || loading?.type === 'realistic_cover' ? <Spinner /> : 
                                                (activeStory.coverImage ? <img src={`data:image/jpeg;base64,${activeStory.coverImage}`} alt="Capa do livro" className="w-full h-full object-cover rounded-lg" /> : <p className="text-xs text-gray-500 p-4 text-center">Nenhuma capa gerada. Use uma das opções acima.</p>)}
                                            </div>
